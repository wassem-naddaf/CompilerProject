...
block : ((begin_end_block | cpp_stmt | stmt ) T_GO?)+ ;               // Multiple consecutive blocks/statements
...
semicolon_stmt :
 ...
 /// ------------------ cpp Section Instructions ------------------- ////

// All CPP Statments
cpp_stmt:   func_cpp_stmt
          | if_stmt
          | for_cpp_stmt
          | assignment_stmt_cpp
          | declare_stmt_cpp
...
// Create  Cpp Function Rule
func_cpp_stmt  : dtype ident  paramsection openscope cpp_body return_stmt T_SEMICOLON closescope;

// Cpp Function Parameters
paramsection : T_OPEN_P parameters* T_CLOSE_P
             ;

parameters :  dtype ident
            | parameters T_COMMA dtype ident
            ;

// Open And Close Scope Brackets
openscope : T_OPEN_B;
closescope : T_CLOSE_B;

// Function Or if  Or LOOP Body
cpp_body :  cpp_stmt*

           ;

// Cpp Assigment Statment
assignment_stmt_cpp : assignment_cpp T_SEMICOLON
                    ;

assignment_cpp :  ident T_EQUAL ident
                | ident T_EQUAL (int_number | dec_number)+
                | assignment_cpp T_COMMA  ident T_EQUAL (int_number | dec_number)+
                | assignment_cpp T_COMMA  ident T_EQUAL ident

                   ;

// Cpp Declare Statment
declare_stmt_cpp : declare_cpp T_SEMICOLON
                 ;

declare_cpp :  dtype ident
             | dtype ident T_EQUAL ((int_number | dec_number)+ | ident)
             | dtype ident T_EQUAL select_stmt
             | declare_cpp T_COMMA ident
             | declare_cpp T_COMMA ident T_EQUAL ((int_number | dec_number)+ | ident)
             ;
 // Cpp if Condition
if_cpp_stmt : T_IF T_OPEN_P bool_expr T_CLOSE_P openscope cpp_body closescope   elseif_cpp_block* else_cpp_block?
             ;

elseif_cpp_block :  T_ELSEIF T_OPEN_P bool_expr T_CLOSE_P openscope cpp_body closescope
                 ;
else_cpp_block :  T_ELSE openscope cpp_body closescope
               ;


// Cpp Loop Statment
for_cpp_stmt : T_FOR T_OPEN_P (assignment_stmt_cpp|declare_stmt_cpp) bool_expr T_SEMICOLON inc_cpp T_CLOSE_P openscope cpp_body closescope;

inc_cpp:  ident T_ADD T_ADD
        | ident T_SUBSUB
        | ident T_ADD T_EQUAL (int_number | ident)
        | ident T_SUB T_EQUAL (int_number | ident)
        | ident T_MUL T_EQUAL (int_number | ident)
        | inc_cpp T_COMMA inc_cpp
...
create_package_stmt :
      ...
      |(T_ALTER | T_CREATE (T_OR T_REPLACE)? | T_REPLACE)? T_PACKAGE ident (T_AS | T_IS) package_spec T_END (ident missing_semicolon)?
    ;
...
create_procedure_stmt :
    ...
    | (T_ALTER | T_CREATE (T_OR T_REPLACE)? | T_REPLACE)? (T_PROCEDURE | T_PROC) ident create_routine_params? create_routine_options? (T_AS | T_IS)? declare_block_inplace? label? proc_block (ident missing_semicolon)?
